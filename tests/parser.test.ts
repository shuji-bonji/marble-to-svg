...
describe('parseMarble', () => {
  it('基本的な文字列をパースできる', () => {
    ...
    expect(result[0].notification.kind).toBe(NotificationKind.NEXT);
    expect(result[0].notification.value).toBe('a');

    // 2番目のイベント（b）
    expect(result[1].frame).toBe(7);
    expect(result[1].notification.kind).toBe(NotificationKind.NEXT);
    expect(result[1].notification.value).toBe('b');

    // 完了イベント
    expect(result[2].frame).toBe(11);
    expect(result[2].notification.kind).toBe(NotificationKind.COMPLETE);
  });
  ...
  it('エラーイベントをパースできる', () => {
    ...
    expect(result[1].notification.kind).toBe(NotificationKind.ERROR);
    if (result[1].notification.kind === NotificationKind.ERROR) {
  ...
  it('空のオブザーバブルをパースできる', () => {
    ...
    expect(result[0].notification.kind).toBe(NotificationKind.COMPLETE);
  });
