...
document.addEventListener('DOMContentLoaded', () => {
  // ビューワーを初期化
  const viewer = new MarbleViewer('marble-viewer-container', DEFAULT_RENDER_OPTIONS);
  
  // サンプル例
  const exampleSelector = document.getElementById('example-selector') as HTMLSelectElement;
  const loadExampleButton = document.getElementById('load-example') as HTMLButtonElement;
